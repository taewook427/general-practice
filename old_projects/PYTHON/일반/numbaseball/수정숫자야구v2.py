...
''')
 ...
 ## 주의 !
달라진 점으로는 전체 질문 중 최대 3번, 
컴퓨터가 거짓말을 할 수 있다는 것입니다.
...
def tusys(userin,stball,chance,lastlist):
    ...
    tu.goto(10,0)
    tu.write(stball,font = ('consolas',80))
    tu.goto(10,-150)
...
def structure():
    ...
    def gamecom():
        ...
        fakestr = ''
        fakestr = fakestr + str( 9 - int(tempstr[3]) )
        fakestr = fakestr + str( 9 - int(tempstr[0]) )
        fakestr = fakestr + str( 9 - int(tempstr[1]) )
        fakestr = fakestr + str( 9 - int(tempstr[2]) )

        print(tempstr,fakestr)
        tusys('****','',12,[ ])
        global faketime
        faketime = 0
        global fakelist
